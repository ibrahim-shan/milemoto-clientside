...
function splitSql(sql: string): string[] {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (!quote && next2 === '--') { while (i < s.length && s[i] !== '\n') i++; buf += '\n'; continue; }
    ...
    if (ch === "'" || ch === '"' || ch === '`') { quote = ch as Quote; buf += ch; continue; }

    if (ch === ';') { const stmt = buf.trim(); if (stmt) out.push(stmt); buf = ''; continue; }
...
async function runFile(filename: string) {
  ...
  try {
    ...
    console.log('RUN:', filename, '| DB=', (dbrow as any)[0]?.db, '| statements=', stmts.length);
...
async function run() {
  ...
  for (const f of files) {
    if (done.has(f)) { console.log('SKIP already applied:', f); continue; }
    await runFile(f);
  }
